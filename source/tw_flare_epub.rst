...
You no longer need an ebook reader like the **Kobo Glo ** to enjoy the technical benefits of an **EPUB** file, but reading from a dedicated ebook reader is arguably healthier for you.

Although ebook readers are no longer as popular as it used to be, the **EPUB** format is still very much in use due to its advantages over PDF. The **XHTML** markup is also so simple, veteran users can take advantage of all the features included with Flare without worrying about output validation and compatibility with mobile apps and ebook applications.
